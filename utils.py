...
def pad_sents_char(sents, char_pad_token):
    ...
    max_word_length = 21
    max_sent_length=max(len(sentence)for sentence in sents)
    ...
    for sentence in sents:
        ...
        for word in sentence:
            if len(word)<max_word_length:
                ...
                word_padded_all.append(word_padded)
            elif len(word)>max_word_length:
                word_padded_all.append(word[:max_word_length])
        if len(word_padded_all)<max_sent_length:
            word_padded_all=word_padded_all+[[char_pad_token]*(max_word_length)]*(max_sent_length-len(word_padded_all))

        sents_padded.append(word_padded_all)
