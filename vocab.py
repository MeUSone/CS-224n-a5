...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        def prepend(word):
            ...
            return word
        word_ids=[[prepend([self.char2id[char]for char in word])for word in sentence]for sentence in sents]
        return word_ids
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        sents_indice=self.words2charindices(sents)
        sents_padded=pad_sents_char(sents_indice,self.char2id['<pad>'])
        sents_padded=torch.tensor(sents_padded)
        return sents_padded.permute([1,0,2])
